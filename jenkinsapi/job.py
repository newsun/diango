        ...
        if scm_url_list and not old_source_url:
            ...
            for scm_url in scm_url_list:
                if scm_url.text == old_source_url: 
                    ...
                    self.update_config(ET.tostring(element_tree))
    def modify_chain(self,new_chain,old_chain=None):
        element_tree = self._get_config_element_tree()
        publishers = element_tree.find('./publishers')
        BuildTrigger = element_tree.find('./publishers/hudson.plugins.parameterizedtrigger.BuildTrigger')
        if BuildTrigger is not None:
            publishers.remove(BuildTrigger)
            # projects = element_tree.find('./hudson.plugins.parameterizedtrigger.BuildTrigger/configs/hudson.plugins.parameterizedtrigger.BuildTriggerConfig/projects')
            # if not old_chain:
                # projects.text = new_chain
                # self.update_config(ET.tostring(element_tree))                        		
            # elif projects.text == old_chain:
                # projects.text = new_chain
                # self.update_config(ET.tostring(element_tree))
        #else:
        if not new_chain:
